...
#   JOINER
    if choice == '1':
        ...
        invite_code = input(f'\n[\x1b[95m>\x1b[95m\x1B[37m] Invite Code: ')
        tokens = open('tokens.txt', 'r').read().splitlines()

        def joiner(token, invite_code):
            headers = mainHeader(token)
            r = requests.post(f'https://discord.com/api/v9/invites/{invite_code}', headers=headers)
            if r.status_code == 200:
                print(f"[{g}>{Fore.RESET}] Successfully Joined Server")
            else:
                print(f"[{lr}>{Fore.RESET}] Failed to Join Server")

        for token in tokens:
            threading.Thread(target=joiner, args=(token, invite_code)).start()
...
#   DM SPAMMER
    if choice == '10':
        ...
        user_id = input(f'\n[\x1b[95m>\x1b[95m\x1B[37m] User ID: ')
        message = input(f'[\x1b[95m>\x1b[95m\x1B[37m] Message: ')
        tokens = open('tokens.txt', 'r').read().splitlines()

        def dm_spammer(token, user_id, message):
            headers = mainHeader(token)
            ...
            if r.status_code == 200:
                ...
                while True:
                    ...
                    if r.status_code == 200:
                        print(f"[{g}>{Fore.RESET}] Sent Message")
                    else:
                        print(f"[{lr}>{Fore.RESET}] Failed to Send")

        for token in tokens:
            threading.Thread(target=dm_spammer, args=(token, user_id, message)).start()
    ...
    if choice == '31':
            ...
            try:
                ...
                if not amount_str.strip():
                    print(f'{Fore.RESET}[{Fore.RED}Error{Fore.RESET}] | Input cannot be empty!')
                    sleep(1.5)
                    spammer()
