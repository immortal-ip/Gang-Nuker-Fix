...
def QR_Grabber(Webhook):
    ...
    if div is None:
        ...
        div = soup.find('div', {'class': lambda x: x and 'qrCode' in x})
    if div is None:
        print(f"[{Fore.RED}Error{Fore.RESET}] QR code div not found. Discord page might have changed.")
        driver.quit()
        sleep(2)
    ...
    img = div.find('img')
    if img is None:
        print(f"[{Fore.RED}Error{Fore.RESET}] QR code img not found inside div.")
        driver.quit()
        sleep(2)
        input("Press Enter to continue...")
        spammer()
